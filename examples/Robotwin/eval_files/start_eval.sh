...
Optional flags:
  ...
  -p, --base-port         First port to allocate (default: 5694, env: ROBOTWIN_BASE_PORT)
      ...
      --external-server-host
                           Use existing policy servers at this host instead of
                           starting local servers (env: ROBOTWIN_EXTERNAL_SERVER_HOST)
      --external-server-count
                           Number of external policy servers. Client slots reuse
                           this server pool in round-robin order.
...
Environment variables (lower priority than flags):
  ...
  ROBOTWIN_EXTERNAL_SERVER_HOST
                              Existing policy-server host; intended for split evaluation
  ROBOTWIN_EXTERNAL_SERVER_COUNT
                              Number of external policy servers shared by client slots
EOF
}
...
server_reachable() {
    local host="$1"
    local port="$2"
    "${ROBOTWIN_PYTHON:-${WALA_PYTHON:-python}}" - "${host}" "${port}" <<'PY' >/dev/null 2>&1
import sys
...
wait_for_server() {
    ...
    while (( elapsed < timeout_s )); do
        if server_reachable "${host}" "${port}"; then
            return 0
        fi
        sleep 2
        elapsed=$((elapsed + 2))
...
launch_task_in_slot() {
    ...
    local policy_host="${EXTERNAL_SERVER_HOST:-127.0.0.1}"
    ...
    (
        ...
        if [[ -z "${EXTERNAL_SERVER_HOST}" ]]; then
            export WALA_PYTHON="${WALA_PYTHON}"
            bash "${SCRIPT_DIR}/run_policy_server.sh" "${CKPT_PATH}" "${gpu_id}" "${port}" > "${server_log}" 2>&1 &
            server_pid=$!
        fi

        if ! wait_for_server "${policy_host}" "${port}" "${ROBOTWIN_SERVER_TIMEOUT:-600}"; then
            if [[ -n "${EXTERNAL_SERVER_HOST}" ]]; then
                echo "[ERROR] External policy server is not reachable for task=${task_name} at ${policy_host}:${port}." >&2
            else
...
opt_port=""
opt_timeout=""
opt_external_server_host=""
opt_external_server_count=""
opt_install=false
...
while (( $# > 0 )); do
    case "$1" in
        ...
        --external-server-host) opt_external_server_host="$2"; shift 2 ;;
        --external-server-count) opt_external_server_count="$2"; shift 2 ;;
...
EXTERNAL_SERVER_HOST="${opt_external_server_host:-${ROBOTWIN_EXTERNAL_SERVER_HOST:-}}"
EXTERNAL_SERVER_COUNT="${opt_external_server_count:-${ROBOTWIN_EXTERNAL_SERVER_COUNT:-}}"
if ${opt_install}; then
...
if [[ -n "${EXTERNAL_SERVER_HOST}" ]]; then
    WALA_PYTHON="${WALA_PYTHON:-${ROBOTWIN_PYTHON}}"
else
...
if [[ -n "${EXTERNAL_SERVER_HOST}" ]]; then
    EXTERNAL_SERVER_COUNT="${EXTERNAL_SERVER_COUNT:-${TOTAL_SLOTS}}"
    if [[ ! "${EXTERNAL_SERVER_COUNT}" =~ ^[1-9][0-9]*$ ]]; then
        echo "External server count must be a positive integer: ${EXTERNAL_SERVER_COUNT}" >&2
        exit 1
    fi
fi
...
for gpu_id in "${CUDA_DEVICES[@]}"; do
    for (( slot_repeat = 0; slot_repeat < JOBS_PER_GPU; ++slot_repeat )); do
        if [[ -n "${EXTERNAL_SERVER_HOST}" ]]; then
            assigned_port=$((BASE_PORT + slot_idx % EXTERNAL_SERVER_COUNT))
        else
...
if [[ -n "${EXTERNAL_SERVER_HOST}" ]]; then
    echo "[INFO] sharing ${EXTERNAL_SERVER_COUNT} external policy servers at ${EXTERNAL_SERVER_HOST}:${BASE_PORT}-$((BASE_PORT + EXTERNAL_SERVER_COUNT - 1))"
fi
    ...
    for (( slot_idx = 0; slot_idx < TOTAL_SLOTS; ++slot_idx )); do
        ...
        if [[ -n "${current_pid}" ]] && ! kill -0 "${current_pid}" 2>/dev/null; then
            ...
            if wait "${current_pid}"; then
                ...
                if [[ -n "${EXTERNAL_SERVER_HOST}" ]]; then
                    echo "[ERROR] Task ${finished_task} failed with status ${exit_code}. See ${finished_eval_log}" >&2
                else
