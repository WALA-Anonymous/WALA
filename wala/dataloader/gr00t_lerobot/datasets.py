...
from wala.dataloader.gr00t_lerobot.v3_data_shard_index import LeRobotV3DataShardIndex
...
def _build_stats_cache_config(
    ...
    state_action_schema: dict | None = None,
    action_keys_full: list[str] | None = None,
    state_keys_full: list[str] | None = None,
    ...
    config = {
        ...
        "state_action_schema": state_action_schema or {},
        "action_keys": list(action_keys_full or []),
        "state_keys": list(state_keys_full or []),
...
def _build_dataset_stats_fingerprint(
    dataset_path: Path,
    lerobot_version: str,
    le_info: dict,
) -> dict:
    return {
        ...
        "codebase_version": le_info.get("codebase_version"),
        "total_episodes": le_info.get("total_episodes"),
        "total_frames": le_info.get("total_frames"),
        "total_tasks": le_info.get("total_tasks"),
        "fps": le_info.get("fps"),
        "data_path": le_info.get("data_path"),
        "video_path": le_info.get("video_path"),
    }


def _build_state_action_stats_schema(
    le_modality_meta: "LeRobotModalityMetadata",
    action_keys_full: list[str],
    state_keys_full: list[str],
) -> dict:
    schema = {}
    for key in list(state_keys_full) + list(action_keys_full):
        meta = le_modality_meta.get_key_meta(key)
...
class LazyStepSequence:
    """Index-only view over dataset steps without materializing every frame index."""

    def __init__(self, dataset):
        self.dataset = dataset

    def __len__(self) -> int:
        return self.dataset.num_steps

    def __getitem__(self, index: int) -> tuple[int, int]:
        return self.dataset.get_step_by_index(index)
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.step_stride = int(self.data_cfg.get("step_stride", 1) if self.data_cfg else 1)
        if self.step_stride < 1:
            raise ValueError(f"step_stride must be >= 1, got {self.step_stride}")
        self.validate_language = bool(self.data_cfg.get("validate_language", True) if self.data_cfg else True)
        ...
        self._random_language_per_sample = (
            self.data_cfg is not None
            and self.data_cfg.get("random_language_per_sample", False) not in ["False", False]
        )
        self._language_task_pool = (
            self._tasks["task"].dropna().astype(str).tolist()
            if self._random_language_per_sample and "task" in self._tasks.columns
            else []
        )
        self.curr_traj_data = None
        self.curr_traj_id = None
        self._v3_data_shard_index = None

        self._trajectory_ids, self._trajectory_lengths = self._get_trajectories()
        self._trajectory_ids_are_sorted = bool(
            len(self._trajectory_ids) < 2
            or np.all(self._trajectory_ids[:-1] <= self._trajectory_ids[1:])
        )
        self._modality_keys = self._get_modality_keys()
        self._delta_indices = self._get_delta_indices()
        (
            self._valid_trajectory_indices,
            self._step_counts,
            self._step_offsets,
            self._num_steps,
        ) = self._get_step_index()
        self._all_steps = LazyStepSequence(self)
        self._depth_cache_dataset_name = _safe_cache_component(self.dataset_name)
        self._depth_cache_dataset_id = hashlib.md5(
            str(self.dataset_path.resolve()).encode()
        ).hexdigest()[:12]
    ...
    def all_steps(self) -> list[tuple[int, int]]:
        """A lazy sequence mapping step indices to ``(trajectory_id, base_index)``."""
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        for new_key in le_modality_meta.video:
            ...
            if "channel" in names:
                ...
                channels = le_video_meta.get("info", {}).get(
                    "video.channels",
                    le_video_meta.get("video_info", {}).get("video.channels", shape[2] if len(shape) > 2 else 3),
                )

            fps = le_video_meta.get("video_info", {}).get(
                "video.fps",
                le_video_meta.get("info", {}).get("video.fps", le_info.get("fps")),
            )
        ...
        dataset_fingerprint = _build_dataset_stats_fingerprint(
            dataset_path=self.dataset_path,
            lerobot_version=self._lerobot_version,
            le_info=le_info,
        )
        state_action_schema = _build_state_action_stats_schema(
        ...
        if needs_low_dim_stats:
            if is_main():
                ...
                parquet_files_filtered = [
                    ...
                    if "episode_033675.parquet" not in pf.name
                ]
        ...
        # v3.0
            ...
            for file_path in file_paths:
                ...
                for index, episode in episodes_data.iterrows():
                    ...
                    video_file_indices = {}
                    for col in video_file_cols:
                        value = episode[col]
                        if pd.isna(value):
                            continue
                        video_key = str(col)[len("videos/") : -len("/file_index")]
                        video_file_indices[video_key] = int(value)
                    video_chunk_indices = {}
                    for col in video_chunk_cols:
                        value = episode[col]
                        if pd.isna(value):
                            continue
                        video_key = str(col)[len("videos/") : -len("/chunk_index")]
                        video_chunk_indices[video_key] = int(value)
                    ...
                    episode_meta = {
                        ...
                        "dataset_from_index": (
                            None
                            if "dataset_from_index" not in episodes_data.columns
                            or pd.isna(episode["dataset_from_index"])
    ...
    def _get_step_index(self) -> tuple[np.ndarray, np.ndarray, np.ndarray, int]:
        """Build a compact index for all valid dataset steps."""

        def is_main():
            return (not dist.is_initialized()) or dist.get_rank() == 0

        config_key = self._get_steps_config_key()
        steps_filename = "steps_lazy_index.pkl"
        steps_path = self._get_dataset_cache_file(steps_filename)

        def _try_load_step_cache():
            ...
            try:
                ...
                print(
                    ...
                    f"Cached step index config mismatch for {steps_path}, will rebuild."
                )
            except Exception as e:
                # include EOFError / PickleError / KeyError
                print(f"[RANK {os.environ.get('RANK', 'NA')}] " f"Failed to load cached step index ({e}), will rebuild.")
            return None

        def _build_and_save_step_cache():
            valid_trajectory_indices, step_counts, step_offsets, total_steps = self._build_step_index_single_process()
            cache_data = {
                "config_key": config_key,
                "valid_trajectory_indices": valid_trajectory_indices,
            ...
            print(f"[RANK 0] Cached lazy step index saved to {steps_path}")
            return cache_data

        cached_data = _try_load_step_cache()
        if cached_data is not None:
            return self._unpack_step_index(cached_data)

        if is_main():
            return self._unpack_step_index(_build_and_save_step_cache())

        # Rank 0 publishes the cache atomically. Other ranks wait for the same
        # cache key without introducing a collective for every subdataset.
        wait_start = time.time()
        wait_timeout = 600
        last_error = None
        while cached_data is None:
            if steps_path.exists():
                try:
                    with open(steps_path, "rb") as f:
                        cached_data = pickle.load(f)
                    if cached_data.get("config_key") != config_key:
                        raise RuntimeError(f"Step index config mismatch after rebuild: {steps_path}")
                except Exception as e:
                    cached_data = None
                    last_error = e
            ...
            if time.time() - wait_start > wait_timeout:
                raise FileNotFoundError(
                    f"Timed out waiting for cached step index at {steps_path}. "
    ...
    def _unpack_step_index(self, cached_data: dict) -> tuple[np.ndarray, np.ndarray, np.ndarray, int]:
        valid_trajectory_indices = np.asarray(cached_data["valid_trajectory_indices"], dtype=np.int64)
        step_counts = np.asarray(cached_data["step_counts"], dtype=np.int64)
        step_offsets = np.asarray(cached_data["step_offsets"], dtype=np.int64)
        total_steps = int(cached_data["total_steps"])
        return valid_trajectory_indices, step_counts, step_offsets, total_steps
    ...
    def _get_steps_config_key(self) -> str:
        ...
        config_dict = {
            ...
            "validate_language": self.validate_language,
            "dataset_name": self.dataset_name,
    ...
    def _build_step_index_single_process(self) -> tuple[np.ndarray, np.ndarray, np.ndarray, int]:
        """Collect valid trajectories and compact per-trajectory step counts."""
        valid_trajectory_indices: list[int] = []
        ...
        has_language_modality = (
            ...
            and "language" in self.modality_keys
            and len(self.modality_keys["language"]) > 0
        )
        for trajectory_index, (trajectory_id, trajectory_length) in enumerate(
            ...
            step_count = (int(trajectory_length) + self.step_stride - 1) // self.step_stride
            if step_count <= 0:
                skipped_trajectories += 1
                continue

            valid_trajectory_indices.append(trajectory_index)
        ...
        if len(step_counts_array) == 0:
            step_offsets = np.asarray([0], dtype=np.int64)
            total_steps = 0
        else:
            step_offsets = np.concatenate(
                [np.asarray([0], dtype=np.int64), np.cumsum(step_counts_array, dtype=np.int64)]
            )
        ...
        print(f"Total steps: {total_steps} from {len(valid_trajectory_indices)} valid trajectories")

        return (
            np.asarray(valid_trajectory_indices, dtype=np.int64),
            step_counts_array,
            step_offsets,
            total_steps,
        )

    def get_step_by_index(self, index: int) -> tuple[int, int]:
        if self.num_steps <= 0:
            raise IndexError(f"Dataset {self.dataset_name} has no steps.")

        if index < 0:
            index += self.num_steps
        if index < 0 or index >= self.num_steps:
            raise IndexError(f"Step index {index} out of range for dataset {self.dataset_name} with {self.num_steps} steps.")

        trajectory_pos = int(np.searchsorted(self._step_offsets, index, side="right") - 1)
        trajectory_index = int(self._valid_trajectory_indices[trajectory_pos])
        base_offset = int(index - self._step_offsets[trajectory_pos])
        base_index = base_offset * self.step_stride
        trajectory_id = self.trajectory_ids[trajectory_index]
        return trajectory_id, base_index
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        """
        trajectory_id, base_index = self.get_step_by_index(index)
    ...
    def _depth_cache_key(self, video_key: str, trajectory_id: int, frame_index: int) -> str:
        return str(
            Path(self._depth_cache_dataset_name)
            / self._depth_cache_dataset_id
            / _safe_cache_component(video_key)
            / f"episode_{_safe_cache_component(trajectory_id)}"
            / f"frame_{int(frame_index):08d}.png"
        )

    def _get_clamped_step_indices(self, key: str, trajectory_id: int, base_index: int) -> np.ndarray:
        step_indices = self.delta_indices[key] + base_index
        trajectory_index = self.get_trajectory_index(trajectory_id)
        step_indices = np.maximum(step_indices, 0)
        step_indices = np.minimum(step_indices, self.trajectory_lengths[trajectory_index] - 1)
        return step_indices.astype(np.int64)

    def _pack_sample(self, data: dict, trajectory_id: int | None = None, base_index: int | None = None) -> dict:
        """Pack transformed modality data into training sample format."""
        all_images = []
        all_depth_cache_keys = []
        ...
        for video_key in video_keys:
            ...
            if trajectory_id is not None and base_index is not None:
                frame_indices = self._get_clamped_step_indices(video_key, trajectory_id, base_index)
            
            video_frames = data[video_key]
            
            past_curr_images = []
            future_images = []
            past_curr_depth_keys = []
            future_depth_keys = []
            ...
            for i, delta in enumerate(delta_indices):
                ...
                if frame_indices is not None:
                    depth_key = self._depth_cache_key(video_key, trajectory_id, int(frame_indices[i]))
                
                if delta <= 0:
                    past_curr_images.append(image)
                    past_curr_depth_keys.append(depth_key)
                else:
                    future_images.append(image)
                    future_depth_keys.append(depth_key)
                
            
            all_images.append([past_curr_images, future_images])
            all_depth_cache_keys.append([past_curr_depth_keys, future_depth_keys])
        ...
        sample = {
            "action": action,
            "image": all_images,
            "depth_cache": all_depth_cache_keys,
            "lang": language,
            "language": language,
        }
        ...
        if parquet_path.exists():
            try:
                ...
                direct_error = ValueError(
                    f"No rows found for episode {trajectory_id} in {parquet_path}"
                )
            except Exception as exc:
                direct_error = exc
        else:
            direct_error = FileNotFoundError(f"Parquet file not found at {parquet_path}")

        dataset_from_index = episode_meta.get("dataset_from_index")
        if dataset_from_index is None:
            assert direct_error is not None
            raise direct_error

        if self._v3_data_shard_index is None:
            expected_total_rows = self.lerobot_info_meta.get("total_frames")
            self._v3_data_shard_index = LeRobotV3DataShardIndex(
                self.dataset_path,
                expected_total_rows=expected_total_rows,
            )

        try:
            return self._v3_data_shard_index.read_episode(
            ...
            raise RuntimeError(
                f"Failed to load LeRobot v3 episode {trajectory_id}. "
                f"Metadata file lookup failed with: {direct_error}. "
                f"Global row lookup failed with: {global_error}"
            ) from global_error
    ...
    def get_trajectory_index(self, trajectory_id: int) -> int:
        ...
        """
        if self._trajectory_ids_are_sorted:
            trajectory_index = int(np.searchsorted(self.trajectory_ids, trajectory_id))
            if (
                trajectory_index < len(self.trajectory_ids)
                and self.trajectory_ids[trajectory_index] == trajectory_id
            ):
                return trajectory_index
            raise ValueError(f"Error finding trajectory index for {trajectory_id}")

        trajectory_indices = np.flatnonzero(self.trajectory_ids == trajectory_id)
        if len(trajectory_indices) != 1:
            raise ValueError(f"Error finding trajectory index for {trajectory_id}, found {trajectory_indices=}")
        return int(trajectory_indices[0])
    ...
    def get_video_path(self, trajectory_id: int, key: str) -> Path:
        ...
        if self._lerobot_version == "v2.0":
            ...
            episode_meta = self.trajectory_ids_to_metadata[trajectory_id]
            video_chunk_index = episode_meta.get("videos/chunk_indices", {}).get(
                original_key, episode_meta["data/chunk_index"]
            )
            video_file_index = episode_meta.get("videos/file_indices", {}).get(
                original_key, episode_meta["data/file_index"]
            )
            video_filename = self.video_path_pattern.format(
    ...
    def get_video(
        ...
        step_indices = self._get_clamped_step_indices(key, trajectory_id, base_index)
    ...
    def get_language(
        ...
        if self._random_language_per_sample and len(self._language_task_pool) > 1:
            language = random.choice(self._language_task_pool)
            return [language] * len(step_indices)
        # Get the annotations
...
class LeRobotMixtureDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Trajectory weights are not used by the active step-level sampler. Build
        # them lazily only if a caller requests the compatibility property.
        self._trajectory_sampling_weights: list[np.ndarray] | None = None
    ...
    def trajectory_sampling_weights(self) -> list[np.ndarray]:
        ...
        if self._trajectory_sampling_weights is None:
            trajectory_sampling_weights = []
            for i, dataset in enumerate(self.datasets):
                weights = np.ones(len(dataset.trajectory_lengths))
                ...
                if weights_sum == 0 or np.isnan(weights_sum):
                    ...
                    weights = np.ones(len(dataset.trajectory_lengths)) / len(dataset.trajectory_lengths)
                else:
                    weights /= weights_sum
                trajectory_sampling_weights.append(weights)

            self._trajectory_sampling_weights = trajectory_sampling_weights
    ...
    def sample_step(self, index: int) -> tuple[LeRobotSingleDataset, int, int]:
        ...
        if dataset_num_steps == 0:
            raise ValueError(f"Dataset {dataset.dataset_name} has no steps.")

        if not self._sequential_step_sampling:
            single_step_index = rng.choice(dataset_num_steps)
        else:
            step_pos = self._step_pos[dataset_index]
            if step_pos >= dataset_num_steps:
                step_pos = 0

            single_step_index = step_pos
            self._step_pos[dataset_index] = step_pos + 1
        trajectory_id, base_index = dataset.get_step_by_index(int(single_step_index))
        return dataset, trajectory_id, base_index
            ...
            if modality == "video":
                ...
                for config in sorted(configs):
                    for key, value in json.loads(config).items():
                        merged_video_config.setdefault(key, value)
                merged_metadata["modalities"][modality] = merged_video_config
                continue
