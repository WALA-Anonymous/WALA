...
class ContinuousTransitionBottleneckTokenizer(nn.Module):
    ...
    def _run_transformer_decoder(
        ...
        if should_checkpoint:
            return checkpoint(
                lambda target, source: module(tgt=target, memory=source),
                tgt,
                memory,
                use_reentrant=False,
            )
        return module(tgt=tgt, memory=memory)
    ...
    def _encode_rgb(self, current_feats: torch.Tensor, future_feats: torch.Tensor) -> torch.Tensor:
        ...
        return self._run_transformer_decoder(self.encoder, queries, memory)
    ...
    def _encode_depth_tokens(
        ...
        return self._run_transformer_decoder(self.depth_encoder, queries, memory)
    ...
    def encode(
        ...
        depth_context = self._run_transformer_decoder(
            self.rgb_depth_cross_fuse,
            rgb_transition_tokens,
            depth_transition_tokens,
        )
    ...
    def _depth_spatial_tokens(
        ...
        *,
        ...
        spatial_tokens = self._run_transformer_decoder(
            self.depth_spatial_decoder,
            spatial_queries,
            transition_tokens,
        )
    ...
    def decode_delta(
        ...
        *,
        ...
        horizon_context = self._run_transformer_decoder(
            self.horizon_decoder,
            horizon_queries,
            transition_tokens,
        )
...
class LatentActionToTransitionTokens(nn.Module):
    ...
    def forward(self, latent_action: torch.Tensor) -> torch.Tensor:
        ...
        if (
            ...
            and self.training
            and torch.is_grad_enabled()
            and any(param.requires_grad for param in self.resampler.parameters())
        ):
            return checkpoint(
                lambda target, source: self.resampler(tgt=target, memory=source),
                query,
                memory,
                use_reentrant=False,
            )
